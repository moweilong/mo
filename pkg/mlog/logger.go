...
func NewLogger(opts *Options, options ...Option) *zapLogger {
	...
	cfg := &zap.Config{
		// 是否在日志中显示调用日志所在的文件和行号，例如：`"caller":"mo/mo.go:75"`
...
func Errorw(msg string, keyvals ...any) { std.Errorw(msg, keyvals...) }
...
func (l *zapLogger) Errorw(msg string, keyvals ...any) { l.z.Sugar().Errorw(msg, keyvals...) }

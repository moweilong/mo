...
func (s *HTTPServer) RunOrDie() {
	mlog.Infow("Start to listening the incoming requests", "protocol", protocolName(s.srv), "addr", s.srv.Addr)
	// 默认启动 HTTP 服务器
	...
	if err := serveFn(); err != nil && !errors.Is(err, http.ErrServerClosed) {
		mlog.Fatalw("Failed to server HTTP(s) server", "err", err)
	}
}

// GracefulStop 优雅地关闭 HTTP 服务器.
func (s *HTTPServer) GracefulStop(ctx context.Context) {
	mlog.Infow("Gracefully stop HTTP(s) server")
	if err := s.srv.Shutdown(ctx); err != nil {
		mlog.Errorw("HTTP(s) server forced to shutdown", "error", err)
	}
}

...
func TimeToTimeString(tm *time.Time) *string {
	...
	return trans.ToPtr(tm.In(GetDefaultTimeLocation()).Format(TimeLayout))
}
...
func TimeToDateString(tm *time.Time) *string {
	...
	return trans.ToPtr(tm.In(GetDefaultTimeLocation()).Format(DateLayout))
}
...
func DurationpbToString(in *durationpb.Duration) *string {
	...
	return trans.ToPtr(in.AsDuration().String())
}
